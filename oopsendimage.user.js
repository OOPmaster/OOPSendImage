...
document.querySelector("#link-oop-if-all").addEventListener("click", oopIFselectAll, false);
document.querySelector("#link-oop-ic-all").addEventListener("click", oopICselectAll, false);
...
function openOOPdiv( imageURL, tag, image_ref, has_next )
{
	tag = tag || '';
	...
	divoop.outerHTML = "<div id='divoop' style='z-index:1000000; position: fixed; text-align: left; height: 200px; width: 600px; padding: 10px; top: "+(h-200)+"px; left:"+(w/2-300)+"px; background: rgba(128,128,255,0.7); font-size: 10px; box-shadow: 0px -5px 27px #000000;; ' >\
	                   <h7 style='font:12px bold Tahoma,Arial,Verdana;' >"+imageURL+"</h7>\
						<div>\
							...
							<br />\
//							"+( has_next ? "next": "<img src='"+imageURL+"' width='150' height='150' style='float: left;'/>" ) +"\
...
function classOOP() {
	...
	this.addUrl = function( url, node, norefresh ){
		...
		document.getElementById( 'oopNUrls' ).innerHTML = this.nUrls;
		if( !norefresh )
		{
    		if( !this.divListe )
    			this.createDiv();
    		
    		// this.divInner.innerHTML += '<span id="oop-link-'+this.nUrls+'" style="display: block;width:20000px;height:15px; overflow: hidden; margin: 1px 0; border: 1px solid #888; background: #aaa;" />'+ this.nUrls +' : ' + url +'</span>';
    		//this.refresh();
	    }
	...
	this.createDiv = function(){
		...
		this.divListe.outerHTML = "<div id='divoopurls' style='z-index:1000000; position: fixed; text-align: left; padding: 10px; width:300px; overflow:hidden; left:0px; top:0px; background: rgba(128,128,255,0.7); font-size: 10px; box-shadow: 0px -5px 27px #000000; ' >\
		                  <a id='oopRefresh'>R</a>\
						AutoTags <input type='text' id='oopTags'  /><button id='oopbt'>GO</button><br />\
						nUrls = <span id='oopNUrls'>0</span>\
		...
		document.getElementById( 'oopRefresh' ).addEventListener("click", oopRefresh2, false);
	}
	...
	this.checkUrl = function(){
		if( !this.encours )
		{
			...
			if( url )
			{
				...
				{
					urlType = 'image';
					openOOPdiv( url, this.getTags(), document.location, this.hasNext() );
				}
				else
				{
					this.getHTML( url );
	...
	this.parseHTML = function( html ){
		...
		if( result )
		{
			...
			urlType = 'image'
			openOOPdiv( urlImage, this.getTags(), this.urlEnCours, this.hasNext() );
		}
		else
			this.finish();
	}
...
function oopIFselectAll()
{
    var selection = document.getSelection();
    var selected = ( selection.anchorNode != selection.focusNode );
	var els = document.querySelectorAll('img');
	for( var e in els )
	{
	   if( !selected || selection.containsNode( els[e], true ) )
	   {
		var url = els[ e ].src;
...
function oopICselectAll()
{
	...
	for( var e in els )
	{
	   ...
	   {
		var url = els[ e ].src;
		if( url.indexOf( 'imgchili' ) != -1 )
		{
    	    //console.log( url );
			url = preg_replace('/t([0-9]+.imgchili)/','i\\1',url);
    		//console.debug( url );
    		OOP.addUrl( url, els[e], true );
		}
       }
...
function oopSelectGalerieLink( )
{
	...
	var tags = prompt( "Tags pour :\n"+link.href, oopOldLinkTags );
	oopOldLinkTags = tags; // sauvegarde les tags pour le prochain passage
